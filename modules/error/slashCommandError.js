...
module.exports = async function handleSlashCommandError(client, interaction, error) {
    try {
        if (interaction.deferred || interaction.replied) {
            await interaction.followUp({ content: `\`/${interaction.commandName}\`の実行中にエラーが発生しました。`, ephemeral: true });
        } else {
            await interaction.reply({ content: `\`/${interaction.commandName}\`の実行中にエラーが発生しました。`, ephemeral: true });
        }
    } catch (followUpError) {
