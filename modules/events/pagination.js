...
const sendButtonErrorLog = require('../error/buttonError'); 
...
async function buttonPages(interaction, pages) {
    ...
    const buttons = [
        ...
        new ButtonBuilder().setCustomId('delete').setLabel('削除').setStyle(ButtonStyle.Danger) // 削除ボタンの追加
    ];
    ...
    collector.on("collect", async (i) => {
        ...
        try {
            ...
            if (error.code !== 50001 && error.code !== 50013) {
                await sendButtonErrorLog(interaction.client, i, error, 'buttonPages', __filename);
            }
        }
    });
    ...
    collector.on('end', async (_, reason) => {
        if (reason !== 'messageDelete' && reason !== 'deleted') { 
            try {
                ...
                if (error.code !== 50001 && error.code !== 50013) {
                    await sendButtonErrorLog(interaction.client, null, error, 'buttonPages', __filename);
                }
            }
        }
    });
